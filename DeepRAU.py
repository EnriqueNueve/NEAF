...
"""
Main object controlling network. All user interaction with
the network goes through this object.
"""
class RAU(object):
    def __init__(self, nodes, Architecture, lr, key_LF, key_Opt,**kwargs):
        # Parameters
        self.lr = lr  # learning rate
        self.key_Opt = key_Opt  # key for optimization
        self.key_LF = key_LF # Intialize loss function
        ...
        # Intialize dropout for needed dense layers
        ...
        Loss_Selector = {"MSE": MSE, "Huber": Huber, "MAE": MAE}
        self.loss_function = Loss_Selector.get(key_LF)
        # END: "Loss Function"

        # BEGIN: Gate activation functions
        ...
        self.NEAF = NEAF
    # END: Gate activation functions

    """
    Description: Clear function used before each training iteration
    """
    def RAU_Clear(self):
        self.previous_outputs = []
        self.Reset_Network_Outputs = []
    ...
    """
    Description: Used to predict on test data
    Pass: (network input data -> np.array)
    Returns: (prediction -> np.array)
    """
    def query(self,inputs):
        self.RAU_Clear() # Clears network's output from previous use
        self.all_outputs = []
        ...
        return self.final_output_stepdown # return prediction


    """
    Description: Used to train network layers
    Pass: (network train input data -> np.array, network train target data -> np.array)
    """
    def train(self, inputs, targets):

        # BEGIN: Part One ~ Foward pass
        self.all_outputs = []
        self.State_Error, self.Past_State_Error = 0, 0
        errors = []
        ...
        for i in range(self.nodes):
            ...
            # Calculate error
            error = np.mean(np.abs(self.final_output_stepdown-targets[i])/targets[i])*100
            errors.append(error)

        print("Percent Error: %", np.round(np.mean(errors),4), "\n")
        ...
        # BEGIN: Part Two ~ Backpropagation
        t = self.nodes - 1
        for i in range(self.nodes):
            ...
            if i == self.nodes: previous_output = 0
            else: previous_output = self.previous_outputs[t]
            ...
            t-=1
        # END: Part Two ~ Back propogation

        # BEGIN: Part Three ~ Update weights
        ...
        self.RAU_Clear()
        self.x += 1
        # END: Part Three ~ Update weights

"""
This object is controlled by the RAU object. This object contains
two networks that make up a single gate. Interaction with the networks
...
class RAU_Gate(object):
    ...
    # Backpropogate error through gate
    ...
    # Update weights of network's in gate
    def RAU_Gate_Update(self):
        self.Network.RAU_Network_Update()
        self.State_Network.RAU_Network_Update()

    # Call networks to implement dropout on network
    def drop(self):
        self.Network.drop()
        self.State_Network.drop()

"""
Multilayer perceptron containing one or more weight layers.
"""
class RAU_Network(object):
    ...
    # Pass data through weight layers
    ...
    # Pass error through weight layers
    ...
    # Clear layers variables for next tranining interation
...
"""
Object containing weight layer.
"""
...
"""
Special object for batchnorm layer.
"""
class BatchNorm():
    ...
    # Foward pass through batchnorm layer
    def foward(self,x):
        self.x = x
    ...
    # Backward pass through batchnorm layer
    def back(self,error,t):
